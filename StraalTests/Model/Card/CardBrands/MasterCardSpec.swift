...
class MasterCardSpec: QuickSpec {
	override func spec() {
		describe("MasterCard") {
			...
			describe("Number validation") {
				...
				it("should return valid for correct MasterCard card number") {
					expect(sut.validate(number: CardNumber(rawValue: "5555 5555 5555 4444"))).to(equal(ValidationResult.valid))
					expect(sut.validate(number: CardNumber(rawValue: "5555555555554444"))).to(equal(ValidationResult.valid))
					expect(sut.validate(number: CardNumber(rawValue: "5555-5555-5555-4444"))).to(equal(ValidationResult.valid))
				}
				...
				it("should return incomplete for incomplete number") {
					expect(sut.validate(number: CardNumber(rawValue: "5555 4444"))).to(equal(ValidationResult.numberIncomplete))
				}

				it("should return too long for too long number") {
					expect(sut.validate(number: CardNumber(rawValue: "5555 4444 4444 444444"))).to(equal(ValidationResult.numberTooLong))
				}

				it("should return luhn test failed for incorrect number") {
					expect(sut.validate(number: CardNumber(rawValue: "5555 4444 4444 4442"))).to(equal(ValidationResult.luhnTestFailed))
				}
			}

			describe("CVV validation") {

				it("should return valid for correct MasterCard CVV") {
